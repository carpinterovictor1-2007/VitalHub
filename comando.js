...
async function uploadFile() {
    const name = prompt('¿Cuál es el nombre del documento?', 'Examen de sangre');
    if (!name?.trim()) return;
    if (isGuest) {
        ...
        try {
            await db.collection('files').doc(currentUser.uid).collection('vault').add({ name: name.trim(), uploaded_at: firebase.firestore.FieldValue.serverTimestamp() });
            await loadFiles();
        } catch (e) { return showToast('⚠️ Error al guardar'); }
    }
...
async function loadFiles() {
    ...
    try {
        ...
        renderFiles(snap.docs.map(d => ({ id: d.id, ...d.data(), uploaded_at: d.data().uploaded_at?.toDate?.().toISOString() || new Date().toISOString() })));
    } catch (e) { renderFiles([]); }
}
...
async function deleteFile(id) {
    ...
    if (isGuest) {
        ...
        try { await db.collection('files').doc(currentUser.uid).collection('vault').doc(String(id)).delete(); await loadFiles(); }
        catch (e) { return showToast('⚠️ Error al eliminar'); }
    }
...
function renderFiles(files) {
    ...
    if (!list) return;
    if (!files?.length) { list.innerHTML=`<div class="empty-state"><i class="fa-solid fa-folder-open"></i><p>No tienes documentos guardados.<br>Toca el botón de arriba para agregar uno.</p></div>`; return; }
    list.innerHTML = files.map(f => {
        const date = new Date(f.uploaded_at).toLocaleDateString('es-ES',{day:'2-digit',month:'short',year:'numeric'});
        return `<div class="file-item"><i class="fa-solid fa-file-medical" style="color:var(--primary);font-size:1.2rem"></i><div style="flex:1"><div style="font-weight:700;font-size:0.9rem">${f.name}</div><div style="font-size:0.75rem;color:var(--text-muted)"><i class="fa-regular fa-calendar"></i> ${date}</div></div><button onclick="deleteFile('${f.id}')" style="background:none;border:none;color:var(--danger);cursor:pointer;padding:0.4rem;border-radius:0.5rem" title="Eliminar"><i class="fa-solid fa-trash-can"></i></button></div>`;
    }).join('');
}
...
async function generateRecipe() {
    ...
    resultDiv.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Generando tu receta...';
    try {
        const res = await fetch('/api/recipe', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ ingredients }) });
        ...
        resultDiv.innerHTML = '<span style="color:var(--danger)">⚠️ Inicia el servidor con <code>node server.js</code> para usar esta función.</span>';
    }
}
